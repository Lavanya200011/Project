...
$sql = "SELECT * FROM donationtable ORDER BY id DESC LIMIT 1"; // Assuming 'id' is the primary key of your table
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $lastRecordName = $row['Name'];
    $lastRecordEmail = $row['Email'];
    $lastRecordNumber = $row['Number'];
    $lastRecordGender = $row['gender'];
    $lastRecordDob = $row['dob'];
    $lastRecordCountry = $row['Country'];
    $lastRecordState = $row['State'];
    $lastRecordCity = $row['City'];
    $lastRecordPincode = $row['Pincode'];
} else {
...
<div class="container">
        ...
        <form id="paymentForm" method="post" action="process_payment.php">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" required value="<?php echo $lastRecordName; ?>">
                <!-- Other form fields -->
            </div>
            <div>
                <label for="phone">Phone.no:</label>
                <input type="text" id="phone" name="phone" required value="<?php echo $lastRecordNumber; ?>">
                <!-- Other form fields -->
            </div>
            
            <div>
                <label for="email">Email:</label>
                <input type="text" id="email" name="email" required value="<?php echo $lastRecordEmail; ?>">
                <!-- Other form fields -->
            </div>
            <div>
