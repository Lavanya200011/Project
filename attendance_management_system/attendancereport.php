  ...
  <div class="bg-white p-8 rounded shadow-lg w-full max-w-lg">
    ...
    <form method="POST" action="generate_report.php" class="space-y-4">
      <div>
        ...
        <select name="dept_id" required class="w-full p-2 border rounded">
          <?php
      ...
      <div>
        ...
        <select name="section_id" required class="w-full p-2 border rounded">
          <?php
          $sections = $conn->query("SELECT * FROM sections");
          while ($row = $sections->fetch_assoc()) {
              echo "<option value='{$row['section_id']}'>Section {$row['section_name']} (Dept_name {$row['dept_name']})</option>";
          }
          ?>
        </select>
      </div>
